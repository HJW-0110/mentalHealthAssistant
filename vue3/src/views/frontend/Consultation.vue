...
// 响应式数据
const currentSession = ref(null)   ///存储当前会话数据
const messages = ref([])
...
// 情绪分析轮询相关
const emotionPollingInterval = ref(null)   //轮训定时器
...
const startAIResponse = async (sessionId, userMessage) => {
  ...
  // 统一的清理函数
  const cleanup = (markComplete = false) => {    //如果没有传递参数，markComplete自动取false
  ...
  try {
    await fetchEventSource('/api/psychological-chat/stream', {
      ...
      signal: ctrl.signal,      //配合 AbortController 使用，支持手动中止流式请求
      ...
      // 接收到服务器推送的消息时触发，event.data 包含消息内容
      onmessage: (event) => {
