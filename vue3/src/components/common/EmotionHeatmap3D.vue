...
const emotionDistribution = computed(() => props.heatmapData.emotionDistribution || {})     //情绪分布统计
...
const init3DHeatmap = () => {
  ...
  renderer.toneMappingExposure = 1.2      //设置色调曝光度
  ...
  scene.add(mainLight)    //向场景添加主光源，用于照亮热力图
...
const createHeatmapBars = () => {
  ...
  const centralSphereGeometry = new THREE.SphereGeometry(centralSphereRadius, 32, 24)  //创建中心大球的几何体，半径为5，32个经度分段，24个纬度分段
  ...
  const centralSphere = new THREE.Mesh(centralSphereGeometry, centralSphereMaterial)   //将centralSphereGeometry, centralSphereMaterial组合成一个网格对象
  centralSphere.position.set(0, 0, 0)
  centralSphere.castShadow = true    //开启该阴影投射功能
  centralSphere.receiveShadow = true    //开启该阴影接收功能
